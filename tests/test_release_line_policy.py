...
def test_identical_status_succeeds():
    ...
    assert proof.branch == "release/build-623-backend-d2f6"
...
def test_8_wrong_branch_in_response_refuses():
    with pytest.raises(rl.AncestryError, match="expected 'release/build-623-backend-d2f6'"):
...
def test_23_repository_and_branch_are_fixed_constants():
    ...
    assert rl.EXPECTED_RELEASE_BRANCH == "release/build-623-backend-d2f6"
    import inspect
...
def test_branch_name_with_a_slash_is_url_encoded():
    ...
    assert "release%2Fbuild-623-backend-d2f6" in branch_calls[0]["url"], branch_calls[0]["url"]
