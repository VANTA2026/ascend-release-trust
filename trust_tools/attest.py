...
from deterministic_archive import exclusion_reason, write_deterministic_targz
...
def reconstruct_source_from_commit(candidate_repo: str | os.PathLike[str], backend_sha: str,
    ...
    # ---- fail closed on any committed approved path the archive filter would drop -------------
    # The filter exists to keep runner-generated bytecode and tool caches out of a byte-reproducible
    # artifact. A path that someone deliberately COMMITTED inside an approved path is not residue:
    # dropping it would ship a tree that differs from the attested commit while every downstream
    # check still agreed, because the build and the reconstruction apply the same filter.
    git_entries = list_approved_entries(candidate_repo, backend_sha, APPROVED_SOURCE_PATHS)
    conflicts = sorted(
        (e.path, exclusion_reason(e.path)) for e in git_entries if exclusion_reason(e.path)
    )
    if conflicts:
        detail = "\n".join(f"    {path}  ({why})" for path, why in conflicts)
        raise AttestationError(
            f"{len(conflicts)} committed path(s) inside the approved source set would be excluded "
            f"by the canonical archive filter. Refusing to ship a tree that differs from the "
            f"attested commit:\n{detail}"
        )
    ...
    # ---- exact set equality across all five representations of the tree ----------------------
    git_paths = {e.path for e in git_entries}
    _require_identical_path_sets(git_paths, export_root, manifest_path, archive_path)

    print(f"  reconstructed from commit {backend_sha} (tree {tree}): sha256={digest}")
    print(f"  path sets equal across git/export/archive/manifest/extract: {len(git_paths)} paths")
    return archive_path, manifest_path, digest


def _require_identical_path_sets(git_paths: set[str], export_root: Path,
                                 manifest_path: Path, archive_path: Path) -> None:
    """Require exact SET equality — not counts — across every representation of the tree.

    Equal counts would let a substitution hide (one path in, one path out). Duplicates would let a
    missing member hide behind an extra one. Only set equality, checked in every direction, closes
    both.
    """
    exported = {
        str(p.relative_to(export_root).as_posix())
        for p in export_root.rglob("*")
        if p.is_file() or p.is_symlink()
    }
    manifest_paths = set(json.loads(manifest_path.read_text())["entries"])

    with tarfile.open(archive_path, "r:gz") as tar:
        members = [m for m in tar.getmembers() if m.isfile() or m.issym()]
    archive_names = [m.name[2:] if m.name.startswith("./") else m.name for m in members]
    archive_paths = set(archive_names)
    if len(archive_names) != len(archive_paths):
        seen, dupes = set(), set()
        for n in archive_names:
            (dupes if n in seen else seen).add(n)
        raise AttestationError(f"archive contains duplicate members: {sorted(dupes)[:5]}")

    for label, observed in (("export", exported), ("archive", archive_paths), ("manifest", manifest_paths)):
        missing = sorted(git_paths - observed)
        extra = sorted(observed - git_paths)
        if missing or extra:
            raise AttestationError(
                f"{label} path set differs from the attested commit's approved paths — "
